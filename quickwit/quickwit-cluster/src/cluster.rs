...
use crate::{ClusterChange, QuickwitService};
...
struct InnerCluster {
    ...
    live_nodes: BTreeMap<ChitchatId, Node>,
...
impl Cluster {
                    ...
                    for change in previous_live_nodes.iter().key_diff(new_live_nodes.iter()) {
                        match change {
                            KeyChange::Added(chitchat_id, _) => {
                                ...
                                let Some(node_state) = chitchat_guard.node_state(chitchat_id) else {
                                    continue;
                                };
                                let node = match Node::try_from_node_state(chitchat_id, node_state)
                                {
                                    Ok(node) => node,
                                    Err(error) => {
                                        error!(
                                ...
                                if node.is_ready() {
                                    // TODO: This would be a good place to test the channel.
                                    info!(
                                        cluster_id=%cluster_guard.cluster_id,
                                        node_id=%chitchat_id.node_id,
                                        "Node `{}` transitioned to ready state.", chitchat_id.node_id
                                    );
                            ...
                            KeyChange::Unchanged(
                                ...
                                if previous_max_version != new_max_version {
                                    let Some(previous_node) = cluster_guard.live_nodes.get(chitchat_id) else {
                                    continue;
                                };
                                    let Some(new_node_state) = chitchat_guard.node_state(chitchat_id) else {
                                    continue;
                                };
                                    ...
                                    if previous_node.is_ready() && !new_node.is_ready() {
                                        warn!(
                                            cluster_id=%cluster_guard.cluster_id,
                                            node_id=%chitchat_id.node_id,
                                            "Node `{}` transitioned out of ready state.", chitchat_id.node_id
                                        );
    ...
    pub async fn ready_nodes(&self) -> Vec<Node> {
        unimplemented!()
    }
    ...
    pub async fn set_self_node_key_value<K: Into<String>, V: Into<String>>(
        &self,
        key: K,
        value: V,
    ) {
    ...
    pub async fn set_self_node_readiness(&self, readiness: bool) {
        ...
        self.set_self_node_key_value(READINESS_KEY, readiness_value)
            .await
    }
...
//         )
//         .await?;
//         cluster2.set_self_node_readiness(true).await;
...
//         )
//         .await?;
//         cluster3.set_self_node_readiness(true).await;
